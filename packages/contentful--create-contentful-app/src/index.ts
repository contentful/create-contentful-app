...
async function initProject(appName: string, options: CLIOptions) {
  ...
  try {
    ...
    if (normalizedOptions.function && normalizedOptions.frontendless) {
  ...
  if (!fs.existsSync(fullAppFolder)) {
    ...
    await generateFunction.nonInteractive({
      ...
      name: functionName,
      keepPackageJson: normalizedOptions.frontendless === true
    } as any);
  }
}
...
(async function main() {
  program
    ...
    .option('--frontendless', 'use with --function to only clone the function as a frontend-less app.')
    .action(initProject);
